...
def manage_ui():
    ...
    return (
        ...
        "document.getElementById('file').onchange=async(e)=>{const files=[...e.target.files];for(const f of files){const fd=new FormData();fd.append('file',f);fd.append('target','media');await fetch('/media/upload',{method:'POST',body:fd});}fetchFiles();};"
        "async function fetchFiles(){const r=await fetch('/media/files');if(!r.ok){alert('List failed');return;}"
        ...
        "const del=document.createElement('button');del.className='btn ghost';del.textContent='Delete';del.onclick=async()=>{if(!confirm('Delete '+f.name+'?'))return;const d=await fetch('/media/delete',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({filename:f.name})});if(d.ok)fetchFiles();else alert('Delete failed');};"
